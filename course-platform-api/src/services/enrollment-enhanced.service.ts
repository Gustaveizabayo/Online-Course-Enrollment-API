...
export class EnrollmentEnhancedService {
  // Enroll in a course
  static async enrollInCourse(userId: string, courseId: string, req?: Request) {
    ...
    EmailService.sendEnrollmentConfirmation(
      ...
      if (result.success) {
        console.log(`í³§ Enrollment confirmation sent to ${user.email}`);
      }
    }).catch(console.error);
  ...
  static async updateProgress(enrollmentId: string, progress: number, req?: Request) {
    ...
    if (completed) {
      ...
      // Send completion notification (could be enhanced)
      EmailService.sendEmail({
        to: enrollment.user.email,
        subject: `í¾‰ Course Completed: "${enrollment.course.title}"`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #10B981;">í¾‰ Congratulations!</h2>
            <p>Hello ${enrollment.user.name},</p>
            <p>You've successfully completed the course <strong>"${enrollment.course.title}"</strong>!</p>
            <p>Great job on your achievement! íº€</p>
            <p>Best regards,<br>The Course Platform Team</p>
          </div>
        `
      }).then(result => {
        if (result.success) {
          console.log(`í³§ Course completion email sent to ${enrollment.user.email}`);
        }
      }).catch(console.error);
    } else {
